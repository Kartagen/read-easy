...
import { Packer } from '@/entities/books/helpers/packer.helper';
...
export class Mobi {
  ...
  async parse() {
    ...
    mobiHeader.exthFlags =
      (mobiHeader.exthFlags & 0x40) === 0x40 ? true : false;
    (_ref5 = new Packer('2H').unpack_from(header, 0xc2)),
      (mobiHeader.firstContentRecord = _ref5[0]),
      (mobiHeader.lastContentRecord = _ref5[1]);
    this.info.title = new Packer(
      '' + mobiHeader.fullNameLength + 's',
    ).unpack_from(header, mobiHeader.fullNameOffset)[0];
    multibyte = 0;
    trailers = 0;
    ...
    return (this.info.content = this.info.content.replace(
      /<(head|HEAD)>/g,
